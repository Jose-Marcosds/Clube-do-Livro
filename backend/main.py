...
def normalizar_autor(nome_raw: str) -> str:
    ...
    if re.search(r'[\u0400-\u04FF]', nome_raw):
        ...
        if "Dostoevsk" in nome_transliterado or "Dostoyevsk" in nome_transliterado:
...
def buscar_capa(q: str):
        ...
        with urllib.request.urlopen(req, context=context) as response:
            ...
            if "items" in data:
                for item in data["items"]:
                    ...
                    cat_final = traduzir_categoria(categorias[0]) if categorias else "Literatura"
    ...
    url_open_library = f"https://openlibrary.org/search.json?q={urllib.parse.quote(q)}&limit=6"
    try:
        ...
        with urllib.request.urlopen(req, context=context) as response:
            ...
            if "docs" in data:
                for doc in data["docs"]:
                    ...
                    capa = f"https://covers.openlibrary.org/b/id/{cover_i}-M.jpg" if cover_i else ""
                    
                    autores = doc.get("author_name", ["Autor Desconhecido"])
                    autor_formatado = normalizar_autor(autores[0])

                    subjects = doc.get("subject", [])
                    cat_final = traduzir_categoria(subjects[0]) if subjects else "Literatura"
...
def cadastrar_livro(livro: LivroCreate):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO livros (titulo, autor, categoria, capa_url) VALUES (?, ?, ?, ?)",
